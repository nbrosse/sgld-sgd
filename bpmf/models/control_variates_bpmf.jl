...
function cv_update_withoutnorm( model::matrix_factorisation, cv::control_variate, tuning::sgld )
    # Calculate required gradient estimates
    ( dlogU, dlogV, dloga, dlogb ) = dlogpost( model, tuning.subsample, model.U, model.V,
                                               model.a, model.b )
    ( dlogUopt, dlogVopt, dlogaopt, dlogbopt ) = dlogpost( model, tuning.subsample, cv.U, cv.V,
                                                           cv.a, cv.b )
    # Update U
    η = sqrt( tuning.ϵ_U ) * reshape(rand( Normal( 0, 1 ),  model.D * model.L ), ( model.D, model.L ))
    model.U += tuning.ϵ_U / 2 * ( cv.ll_U - dlogUopt + dlogU ) + η
    # Update V
    η = sqrt( tuning.ϵ_V ) * reshape(rand( Normal( 0, 1 ), model.D * model.M), model.D, model.M )
    model.V += tuning.ϵ_V / 2 * ( cv.ll_V - dlogVopt + dlogV ) + η
    # Update a
    η = sqrt( tuning.ϵ_a ) * rand( Normal( 0, 1 ), model.L )
    model.a += tuning.ϵ_a / 2 * ( cv.ll_a - dlogaopt + dloga ) + η
    # Update b
    η = sqrt( tuning.ϵ_b ) * rand( Normal( 0, 1 ), model.M )
    model.b += tuning.ϵ_b / 2 * ( cv.ll_b - dlogbopt + dlogb ) + η
...
function sgld_update( model::matrix_factorisation, tuning::sgld )
    ...
    println("grad ", mean(abs(tuning.ϵ_U / ( sqrt( tuning.G_U ) * 2 ) * dlogU)))
    println("gaussian noise ", mean(abs(η)))
    # Update V
...
function sgld_update_withoutnorm( model::matrix_factorisation, tuning::sgld )
    ...
    # Update U
    η = sqrt( tuning.ϵ_U ) * reshape(rand( Normal( 0, 1 ),  model.D * model.L ), ( model.D, model.L ))
    model.U += tuning.ϵ_U / 2 * dlogU + η
    # Update V
    η = sqrt( tuning.ϵ_V ) * reshape(rand( Normal( 0, 1 ), model.D * model.M), model.D, model.M )
    model.V += tuning.ϵ_V / 2 * dlogV + η
    # Update a
    η = sqrt( tuning.ϵ_a ) * rand( Normal( 0, 1 ), model.L )
    model.a += tuning.ϵ_a / 2 * dloga + η
    # Update b
    η = sqrt( tuning.ϵ_b ) * rand( Normal( 0, 1 ), model.M )
    model.b += tuning.ϵ_b / 2 * dlogb + η
...
function sgd_mcmc_full_update( model::matrix_factorisation, tuning::sgld)
    tuning.subsample = sample( 1:model.N, tuning.subsize )
