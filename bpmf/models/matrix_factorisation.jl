...
type matrix_factorisation
    ...
    dlogU::Array{ Float64, 2 }              # Latent feature vector
    dlogV::Array{ Float64, 2 }              # Latent feature vector
    dloga::Array{ Float64, 1 }              # User bias terms
    dlogb::Array{ Float64, 1 }              # Item bias terms
...
function update_Λ( model::matrix_factorisation )
    for d in 1:model.D
        dens_current = Gamma( model.α + model.L/2, (model.β + 1/2 * sum( model.U[d,:] .^ 2 ))^(-1) )
        model.Λ_U[d] = rand( dens_current )
        dens_current = Gamma( model.α + model.M/2, (model.β + 1/2 * sum( model.V[d,:] .^ 2 ))^(-1) )
        model.Λ_V[d] = rand( dens_current )
    end
    dens_current = Gamma( model.α + model.L/2, (model.β + 1/2 * sum( model.a .^ 2 ))^(-1) )
    model.λ_a = rand( dens_current )
    dens_current = Gamma( model.α + model.M/2, (model.β + 1/2 * sum( model.b .^ 2 ))^(-1) )
...
Calculate gradient estimate of logposterior
"""
function dlogpostfull( model::matrix_factorisation,
                   U_curr::Array{Float64,2}, V_curr::Array{Float64,2},
                   a_curr::Array{Float64,1}, b_curr::Array{Float64,1} )
    # Initialise containers for gradients
    dlogU = zeros( model.D, model.L )
    ...
    println()
    # Add log prior gradient estimate to containers
    dlogpriorUfull( model, dlogU, U_curr )
    dlogpriorVfull( model, dlogV, V_curr )
    dlogpriorafull( model, dloga, a_curr )
    dlogpriorbfull( model, dlogb, b_curr )
    return ( dlogU, dlogV, dloga, dlogb )
end
...
Gradient of the log prior wrt U
"""
function dlogpriorUfull( model::matrix_factorisation,
                     dlogU::Array{Float64,2}, U_curr::Array{Float64,2} )
    for user in 1:model.L
        for d in model.D
            dlogU[d,user] -= model.Λ_U[d] * U_curr[d,user]
        end
    end
end
...
Gradient of the log prior wrt V
"""
function dlogpriorVfull( model::matrix_factorisation,
                     dlogV::Array{Float64,2}, V_curr::Array{Float64,2} )
    for item in 1:model.M
        for d in model.D
            dlogV[d,item] -= model.Λ_V[d] * V_curr[d,item]
        end
    end
end
...
Gradient of the log prior for user bias a
"""
function dlogpriorafull( model::matrix_factorisation,
                     dloga::Array{Float64,1}, a_curr::Array{Float64,1} )
    for user in 1:model.L
        dloga[user] -= model.λ_a * a_curr[user]
    end
end
...
Gradient of the log prior for item bias b
"""
function dlogpriorbfull( model::matrix_factorisation,
                     dlogb::Array{Float64,1}, b_curr::Array{Float64,1} )
    for item in 1:model.M
        dlogb[item] -= model.λ_b * b_curr[item]
    end
end
